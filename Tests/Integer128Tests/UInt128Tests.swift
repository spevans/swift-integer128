...
final class UInt128Tests: XCTestCase {
    ...
    func testStrings() throws {
        ...
        for digit in digits {
            for length in 1..<UInt128.bitWidth / 4 {
                let string = String(repeating: digit, count: length)
                let value = UInt128(string, radix: 16)
    ...
    func testMiscProperties() throws {
        ...
        XCTAssertEqual(UInt128.zero.loBits, 0)
        XCTAssertEqual(UInt128.zero.hiBits, 0)
    ...
    func testBitOperators() {
        ...
        for data in testData {
            ...
            var result2 = number1
            op2(&result2, number2)
            XCTAssertEqual(result2.hiBits, data.5)
    ...
    func testWords() {
        ...
        for data in testData {
            let number = UInt128(data.0) << 64 | UInt128(data.1)
